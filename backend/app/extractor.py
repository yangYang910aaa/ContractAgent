...
EXTRACT_LABELS: dict[str, str] = {
    ...
    # X% 为限"); 只约束违约金的句子不算赔偿责任上限, 填 null.
    # 实测被误抽的写法："违约金最高为合同金额的 5%""违约金一般为监理合同总价的 20%"
    # ——那两句约束的是违约金，填进责任上限会判出"上限过低"并误停闸。
    "liability_cap": "赔偿责任上限（占合同总额 %，如'赔偿总额以合同总价的 X% 为限'）。"
    "只填约束'赔偿责任/责任总额'的约定；'违约金最高为合同金额的 X%'"
    "'违约金一般为合同总价的 X%'这类只约束违约金的写法不要填，填 null",
