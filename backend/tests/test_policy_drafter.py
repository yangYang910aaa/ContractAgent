...
from backend.app import policy_assistant, policy_drafter
...
def test_render_matches_corpus_style() -> None:
    ...
    text = policy_drafter.render_ai_draft(_DRAFTED, ref="P-16", group="集团采购管理中心")
    ...
    parsed = policy_assistant.parse_policy(text, source="P-16_AI起草.md")
...
def test_new_numbers_flags_only_invented_values() -> None:
    ...
    text = policy_drafter.render_ai_draft(_DRAFTED, ref="P-16")
    findings = policy_drafter.new_numbers("预付款不得超过合同总额的 30%。", text)
    assert findings and findings[0]["article"].startswith("第")
    assert "40" in findings[0]["percent"] and "30" not in findings[0]["percent"]
    # 需求里给过 40% 时就不再提示
    assert policy_drafter.new_numbers("比例 40%，按合同总额计算。", text) == []
...
def test_draft_policy_leaves_unknown_meta_blank(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(policy_assistant, "DRAFTS_DIR", tmp_path / "drafts")
    result = policy_drafter.draft_policy(_BRIEF, drafter=_drafter, retriever=lambda query, k: [])
